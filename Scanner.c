...
jamesr_intg line;								/* Current line number of the source code */
extern jamesr_intg errorNumber;				/* Defined in platy_st.c - run-time error number */

extern jamesr_intg stateType[NUM_STATES];
extern jamesr_string keywordTable[KWT_SIZE];
extern PTR_ACCFUN finalStateTable[NUM_STATES];
extern jamesr_intg transitionTable[NUM_STATES][CHAR_CLASSES];
...
jamesr_intg startScanner(BufferPointer psc_buf) {
	...
	for (jamesr_intg i=0; i<NUM_TOKENS;i++)
...
Token tokenizer(jamesr_void) {
	...
	jamesr_char c;	/* input symbol */
	jamesr_intg state = 0;		/* initial state of the FSM */
	jamesr_intg lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	jamesr_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	jamesr_intg lexLength;		/* token length */
	jamesr_intg i;				/* counter */
	///jamesr_char newc;			// new char
...
jamesr_intg nextState(jamesr_intg state, jamesr_char c) {
	jamesr_intg col;
	jamesr_intg next;
	col = nextClass(c);
...
jamesr_intg nextClass(jamesr_char c) {
	jamesr_intg val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
