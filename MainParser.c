...
jamesr_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern jamesr_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern jamesr_intg line; /* source code line number - defined in scanner.c */
...
extern jamesr_void startParser(jamesr_void);
extern jamesr_intg startScanner(BufferPointer sc_buf);

static jamesr_void printParserError(jamesr_string fmt, ...);
static jamesr_void displayParser(BufferPointer ptrBuffer);
static jamesr_long getParserFileSize(jamesr_string fname);
static jamesr_void callGarbageCollector(jamesr_void);
...
jamesr_intg mainParser(jamesr_intg argc, jamesr_string* argv) {

	numParserErrors = 0;			/* Initializes the errors */

	FILE* fi;       /* input file handle */
	jamesr_intg loadsize = 0; /*the size of the file loaded in the buffer */
...
jamesr_void printParserError(jamesr_string fmt, ...) {

	va_list ap;
	va_start(ap, fmt);

	(jamesr_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
...
jamesr_long getParserFileSize(jamesr_string fname) {
	FILE* input;
	jamesr_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
jamesr_void displayParser(BufferPointer ptrBuffer) {
...
jamesr_void callGarbageCollector(jamesr_void) {
